...
# NOTE: here just enable iam logger, logger level is debug temporarily
#       should change to logging.ERROR in production
import sys
import logging
...
class Permission(object):
    ...
    def _make_request_with_resources(self, username: str, action_id: str, resources: List[Resource]) -> Request:
        request = Request(
            settings.BK_IAM_SYSTEM_ID,
            Subject("user", username),
            Action(action_id),
    ...
    def make_action_application(self, action_id: str) -> Application:
        action = ActionWithoutResources(action_id)
        actions = [action]
        application = Application(settings.BK_IAM_SYSTEM_ID, actions)
        return application

    def make_task_application(self, task_id: str) -> Application:
    ...
    def generate_apply_url(self, application: Application, bk_username: str = "") -> str:
        """
        处理无权限 - 跳转申请列表
        """
