...
pageRouter.get('*', async (ctx, next) => {
    const xScriptName = ctx.request.headers['x-script-name'] || ''
    const publicPath = xScriptName.endsWith('/') ? xScriptName.slice(0, -1) : xScriptName
    renderParams.BK_STATIC_URL = publicPath
    renderParams.SITE_URL = publicPath
